...
class TestDeploymentProfiles:
    ...
    async def test_full_profile_workflow(self):
        ...
        with patch("chromadb.HttpClient", return_value=mock_chroma_client), patch(
            "neo4j.GraphDatabase.driver", return_value=mock_neo4j_driver
        ), patch.dict(
            "os.environ",
            {
...
class TestErrorRecoveryWorkflows:
    ...
    async def test_database_failure_recovery(self):
        ...
        with patch(
            "duckdb.connect", side_effect=Exception("Database connection failed")
        ):
